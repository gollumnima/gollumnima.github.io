...
우리 허선생님은 찰떡같이 가르치셨으나, 학생(나 ㅋㅋㅋ)이 못 알아듣고 왜곡해서 이상한 말을 썼을 수도 있음을..👉👈
...
일반적인 웹서버를 생각해보자.<br/>
푸드트럭에서 일하는 사람이 전자레인지에서 음식 데펴서 손님에게 주는 것<br/>
전자렌지를 돌리는 동안 똘똘한 사람들이라면 다른 음식을 조리하거나 음식을 주문받을텐데 비효율적으로 작동하는 방식이다.
